...
- **What the six sections are:** `references/flatten/output.md`
- **How everything looks:** the templates. `assets/artifact.html` and `assets/reasoning.html`
  carry the type, the colours and the print rules; `assets/figures.html` carries the drawings.
...
list carries well. That is the whole vocabulary: five drawings in `{root}/assets/figures.html`.
...
**`simple-strategy-artifact.html`** from `assets/artifact.html`. The six sections, four slots.

**`reasoning.html`** from `assets/reasoning.html`. Four parts, and the first three may not be empty:
