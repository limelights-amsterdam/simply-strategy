...
| 3 | **What we stop doing** | The sacrifice, per must-solve, and why this side: one sentence per stop on what the plan gains by it, when the material gives a reason | Yes |
...
| 3 · What we stop doing | Four stops and four starts, the ones that block a decision, each stop with the plan's reason for it or the line that it gives none | All of them, with a pointer each |
