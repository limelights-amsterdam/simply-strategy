...
The intake that steers the run. It is filled once per client, not once per run, and it
...
Start from [compass-template.md](compass-template.md).
...
**Read it from material.** The better mode, and the one to offer first. Read
...
**Ask five questions.** When the folder is empty or the user has no deck yet. Use `AskUserQuestion`,
and put anti-vision and boundaries first.
...
[compass-fields.md](compass-fields.md).
